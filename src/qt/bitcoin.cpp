...
bool ThreadSafeAskFee(int64 nFeeRequired, const std::string& strCaption, unsigned char cUnit)
{
    ...
    {
        LOCK(cs_main);
        nMinFee = pindexBest->GetSafeMinFee(cUnit);
    }
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. Nu can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
//		app.setApplicationName("Nu-Qt");
	...
	printf("bitcoin.cpp,main,AppInit2()\n");
    try
    {
        BitcoinGUI window;
        guiref = &window;
