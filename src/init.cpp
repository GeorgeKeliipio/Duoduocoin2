...
{
    bool fRet = false;
	printf("init.cpp,Appinit,AppInit2()\n");
    try
    {
...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("DDS version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  nud [options]                   \t  " + "\n" +
          ...
          _("Options:") + "\n" +
            ...
            "  -port=<port>     \t\t  " + _("Listen for connections on <port> (default: 7890 or testnet: 7895)") + "\n" +
            ...
            "  -rpcport=<port>  \t\t  " + _("Listen for JSON-RPC connections on <port> (default: 14001 or testnet: 15001)") + "\n" +
    ...
    //
    if (fDaemon)
        fprintf(stdout, "DDS server starting\n");
    int64 nStart;
    ...
    BOOST_FOREACH(unsigned char unit, sAvailableUnits)
    {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            {
                strErrors << _("Wallet needed to be rewritten: restart DDS to complete") << "\n";
...
//	if (!CreateThread(CheckThread, NULL))	{
//		printf("Error: CreateThread(CheckThread) failed\n");
//	}
