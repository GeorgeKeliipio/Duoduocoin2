...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DDS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DDS is downloading blocks...");
...
Value burn(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "<amount> is a real and is rounded to the nearest 0.0001\n"
            "<unit> is the unit to burn ('B' for DD Dollar, 'S' for DD Shares).\n"
            "requires portfolio passphrase to be set with walletpassphrase first");
