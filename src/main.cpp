...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "DDS", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        unsigned int nTimeGenesis=1407023435;//1440181098;//1407023435;
        ...
        if (fTestNet)
        {
            ...
            nTimeGenesis=1407023435;//1440181098;//1407023435;
        ...
        printf("DDS Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End DDS Genesis Block\n");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // nu: immediate retry during initial download
...
{
#ifdef TESTING
	printf("MingLang:fProofOfStake:%d,fGenerateSingleBlock:%d\n",fProofOfStake,fGenerateSingleBlock);
#endif
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            Sleep(1000);
            if (fShutdown)	{
				printf("exit:fShutdown\n");
				return;
            }
            if ((!fGenerateBitcoins) && !fProofOfStake)	{
				printf("exit:(!fGenerateBitcoins) && !fProofOfStake\n");
				return;
            }
        }
...
#else
				printf("Dennis:!fGenerateSingleBlock mode\n");
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
        ...
        loop
        {
            ...
            if (!fGenerateBitcoins)	{
				printf("exit:!fGenerateBitcoins\n");
				return;
            }
            if (fLimitProcessors && vnThreadsRunning[THREAD_MINER] > nLimitProcessors)	{
				printf("exit:fLimitProcessors && vnThreadsRunning[THREAD_MINER] > nLimitProcessors\n");
				return;
            }		
            if (vNodes.empty())	{
                break;
...
//        BitcoinMiner(pwallet, true);
	...
	Sleep(3600);
	printf("MingLang:after Sleep(3600)\n");
}
