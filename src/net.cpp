...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "DDS" + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
//	    addrman.Add(vAdd, CNetAddr("120.25.147.12"));
        }
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Nu is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    if (!GetBoolArg("-stakegen", true))	{
		printf("return:!GetBoolArg('-stakegen', true)\n");
		return;
    }
#endif	
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadNuMiner still running\n");
...
{
	printf("MingLang:CheckThread\n");
while (1)
{	
	sleep(3600);
	if(0==vnThreadsRunning[THREAD_MINTER])
	{
#if	1	
	    if (semOutbound == NULL) {
	        // initialize semaphore
		...
		if(0==vnThreadsRunning[THREAD_DNSSEED])	{
			printf("MingLang:ThreadDNSAddressSeed\n");
		    if (!GetBoolArg("-dnsseed", true))
		        printf("DNS seeding disabled\n");
		    else
		        if (!CreateThread(ThreadDNSAddressSeed, NULL))
	    ...
	    // Map ports with UPnP
	    if(0==vnThreadsRunning[THREAD_UPNP]){
			printf("MingLang:MapPort\n");
		    if (fHaveUPnP)
		        MapPort(fUseUPnP);
	    }
	    ...
	    // Send and receive from sockets, accept connections
	    if(0==vnThreadsRunning[THREAD_SOCKETHANDLER])	{
			printf("MingLang:ThreadSocketHandler\n");
		    if (!CreateThread(ThreadSocketHandler, NULL))
		        printf("Error: CreateThread(ThreadSocketHandler) failed\n");
	    }

	    // Initiate outbound connections from -addnode
	    if(0==vnThreadsRunning[THREAD_ADDEDCONNECTIONS]) {
			printf("MingLang:ThreadOpenAddedConnections\n");
		    if (!CreateThread(ThreadOpenAddedConnections, NULL))
		        printf("Error: CreateThread(ThreadOpenAddedConnections) failed\n");
	    }

	    // Initiate outbound connections
	    if(0==vnThreadsRunning[THREAD_OPENCONNECTIONS])	{
			printf("MingLang:ThreadOpenConnections\n");
		    if (!CreateThread(ThreadOpenConnections, NULL))
		        printf("Error: CreateThread(ThreadOpenConnections) failed\n");
	    }
	    // Process messages
	    if(0==vnThreadsRunning[THREAD_MESSAGEHANDLER]){
			printf("MingLang:ThreadMessageHandler\n");
		    if (!CreateThread(ThreadMessageHandler, NULL))
		        printf("Error: CreateThread(ThreadMessageHandler) failed\n");
		}

	    // Dump network addresses
	    if(0==vnThreadsRunning[THREAD_DUMPADDRESS])	{
			printf("MingLang:ThreadDumpAddress\n");
		    if (!CreateThread(ThreadDumpAddress, NULL))
		        printf("Error; CreateThread(ThreadDumpAddress) failed\n");
	    }
	    // Generate coins in the background
	    if(0==vnThreadsRunning[THREAD_MINER])	{
			printf("MingLang:GenerateBitcoins\n");
	   	 	GenerateBitcoins(GetBoolArg("-gen", false), GetWallet('S'));
	    }
	...
	//    StartUpdateFromDataFeed();
		...
		if(0==vnThreadsRunning[THREAD_MINTER])	{
			printf("MingLang:ThreadStakeMinter\n");
		    if (!CreateThread(ThreadStakeMinter, GetWallet('S')))
		        printf("Error: CreateThread(ThreadStakeMinter) failed\n");
		}
	}
}
printf("exit.CheckThread\n");
}
